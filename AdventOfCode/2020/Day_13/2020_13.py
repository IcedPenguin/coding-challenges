...
###################################################################################################################################################################
#  
#  Solution to day 13 part 1: 1915
#
#  Solution to day 13 part 2: 294354277694107
#
###################################################################################################################################################################
...
sample_input_file_1 = "2020_13_sample_1.txt"
sample_input_file_2 = "2020_13_sample_2.txt"
input_file          = "2020_13_input.txt"
...
def read_bus_schedule(file_name):
    ...
    lines[1] = lines[1].replace(",x", "") # breaks if x is first item in input
    bus_ids = [int(x) for x in lines[1].split(",")]
...
def find_earliest_departing_bus(earliest_timestamp, bus_ids):
    earliest_bus_found = False
    current_timestamp = earliest_timestamp

    while not earliest_bus_found:
        print("checking time {0}".format(current_timestamp))
        for bus_id in bus_ids:
            if current_timestamp % bus_id == 0:
                return bus_id, current_timestamp - earliest_timestamp

        current_timestamp += 1

### find_earliest_departing_bus



print("--- P1 sample input ---")
earliest_timestamp, bus_ids = read_bus_schedule(sample_input_file_1)
bus_id, minutes_to_wait = find_earliest_departing_bus(earliest_timestamp, bus_ids)
test_equal(bus_id * minutes_to_wait, 295, "P1 failed")
print("-------------------------")



earliest_timestamp, bus_ids = read_bus_schedule(input_file)
bus_id, minutes_to_wait = find_earliest_departing_bus(earliest_timestamp, bus_ids)

print("Solution to day 13 part 1: {0}".format(bus_id * minutes_to_wait))
...
def chinese_remainder(n, a):
    ...
    print("sum={0} \tprod={1}".format(sum, prod))
    return sum % prod
...
def read_bus_schedule_2(file_name):
    lines = []
    with open(file_name) as f:
        for line in f:
            lines.append(line.strip())
    ...
    for bus_idx in range(len(parts)):
        if parts[bus_idx] == "x":
            continue

        bus_ids.append( int(parts[bus_idx]) )
        bus_offsets.append(int(parts[bus_idx]) - bus_idx)
        # bus_offsets.append(bus_idx)

    return bus_ids, bus_offsets
### read_bus_schedule_2


print("--- P2 sample input ---")
bus_ids, bus_offsets = read_bus_schedule_2(sample_input_file_1)

# print(bus_ids)
# print(bus_offsets)

timestamp = chinese_remainder(bus_ids, bus_offsets)
print(timestamp)


print("\n some simple cases")
# timestamp = chinese_remainder([7,13], [0,5])
# timestamp = chinese_remainder([5,7], [0,1])

# timestamp = chinese_remainder([15,19], [0,1])


timestamp = chinese_remainder([7,13,59,31,19], [0, 12, 55, 25, 12])
# timestamp = chinese_remainder([13, 7], [1, 0])
print(timestamp)
# print(timestamp + 5)
...
bus_ids, bus_offsets = read_bus_schedule_2(input_file)
timestamp = chinese_remainder(bus_ids, bus_offsets)

print("Solution to day 13 part 2: {0}".format(timestamp))
