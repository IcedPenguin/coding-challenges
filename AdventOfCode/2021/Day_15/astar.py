...
class Node:
    ...
    def __init__(self, x, y, risk_sore=0):

        self.x = x 
        self.y = y
        self.risk_sore = risk_sore
        self.f = 0
        self.g = risk_sore
        self.h = 0
        self.neighbors = []
    ...
    def __repr__(self):
        return "Node[x={0}, y={1}, g={2}, n={3}] ".format(self.x, self.y, self.risk_sore, len(self.neighbors))
...
class AStar:
    ...
    @staticmethod
    def load_chiton_map(input_file):
        with open(input_file) as f:
            raw_chiton_map = f.read().splitlines()

        height = len(raw_chiton_map)
        width = len(raw_chiton_map[0])

        grid = []
        for y in range(height):
            grid.append([])
            for x in range(width):
                grid[y].append(Node(x,y, int(raw_chiton_map[y][x])))

        # print("raw map")
        # print(raw_chiton_map)
    ...
    #     # NOTE:  this is literally the structure of the input file.
    ...
    #     # NOTE: grid is filled with risk scores
    ...
    def start_path(open_set, closed_set, current_node, end):
        ...
        for neighbor in neighbors:
            if (neighbor in closed_set) or (neighbor.obstacle == True):
                ...
                temp_g = current_node.g + neighbor.g
                control_flag = 0
...
# ---------- impl to learn how this works ---------- #
star = AStar( cols, rows, start, end)
grid, height, width = star.load_chiton_map("2021_sample_2.txt")
# grid, height, width = star.load_chiton_map("2021_sample_1.txt")
grid = AStar.get_neighbors(grid, width, height)
# print(grid)
...
# grid = star.load_chiton_map("2021_input.txt")
